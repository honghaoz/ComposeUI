...
- Bounds a render handler or a renderable callback changes during the view's own render pass, for example a resize from `onDidRender`, now render after the pass. Before, only a size the `onWillRender` handler changed did.
