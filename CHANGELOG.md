...
- A non-animated `DropShadowLayer` or `InnerShadowLayer` update now continues in-flight shadow and mask animations toward the new values, instead of letting an earlier animation finish towards its old values first and then snapping: an animating opacity, radius, or offset glides to the new value, and an animating color or path is retargeted from what the layer currently shows and lands when it would have. Pass an animation timing to animate the change itself.
- A non-animated `ColorNode` update retargets an in-flight background color animation to the new color the same way.
- Added `CALayer.retarget(keyPath:to:)`, which sets a non-additive property, for example a color or a path, and retargets its in-flight animations to the new value instead of leaving them heading to the old one or cutting them.
