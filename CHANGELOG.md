...
- The `backgroundColor`, `opacity`, `border`, `cornerRadius` and `shadow` modifiers and `ColorNode` now animate only the properties that changed, like the shadow layers. A non-animated update of the modifiers now continues their in-flight animations toward the new values. The `shadow` modifier's opacity animates non-additively.
