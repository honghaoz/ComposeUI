...
- Fixed a crash on macOS when `CALayer.retarget(keyPath:to:)` or `CALayer.animate` set a component of a view-backed layer's geometry, such as `position.x` or `bounds.size.width`. Setting a view-backed layer's whole `bounds` now keeps the view's frame in sync too.
