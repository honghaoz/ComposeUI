...
- When a render handler or a renderable callback changes the view's bounds during its own render pass, for example a resize from `onDidRender`, the new bounds now render after the pass. Before, only a size change from `onWillRender` did.
