...
- The `backgroundColor`, `opacity`, `border`, `cornerRadius` and `shadow` modifiers and `ColorNode` now animate only the properties that changed, like the shadow layers. The `opacity` modifier also resets a view's alpha when its renderable is reused.
